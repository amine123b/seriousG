...
# Connexion MongoDB
client = MongoClient("mongodb+srv://admin:password4321@seriousgame.a547k.mongodb.net/")
...
# Identifiants admin (simulés avec hash pour la sécurité)
ADMIN_CREDENTIALS = {
    "admin": hashlib.sha256("password123".encode()).hexdigest()
}

# Vérification du login administrateur
def check_login(username, password):
    hashed_password = hashlib.sha256(password.encode()).hexdigest()
    return username in ADMIN_CREDENTIALS and ADMIN_CREDENTIALS[username] == hashed_password
...
# 🔥 **PAGE LEADERBOARD**
    ...
    if not games:
        ...
        transport = st.number_input("Distance de transport (km)", min_value=0.0, step=1, key="transport")
        ...
        if st.button("Calculer l'empreinte carbone"):
            ...
            collection.update_one(
                ...
                {"$set": {"carbone": carbone_total, "joueurs": joueurs, "carbone_par_joueur": carbone_par_joueur}}
            )
